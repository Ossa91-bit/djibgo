...
serve(async (req) => {
  ...
  try {
    ...
    const tempEmail = `pro_${phone.replace(/\s+/g, '')}@temp.servicedj.com`;
    ...
    // Créer le profil professionnel avec le bon nom de champ (id au lieu de user_id)
    ...
    // Créer une notification pour le nouveau professionnel
    await supabaseAdmin.from('notifications').insert({
      user_id: userId,
      title: 'Bienvenue sur ServiceDJ',
      message: 'Votre compte professionnel a été créé par un administrateur. Vous pouvez maintenant recevoir des réservations.',
      type: 'info',
